...
//Adaptive DQN
const int ADA_ACTIONS       = 11;    // 0~10 * 0.1
...
int main(int argc, char** argv) {
  ...
  ReplayMemory* exp[4]                = {new ReplayMemory(MEM_SIZE)}; /////////////////////////////////// multi 1
  ...
  std::ofstream output_file(log_file_name); ///// need to be edit
  */

  std::string log_file_name[NUM_OUTPUT], model_name[NUM_OUTPUT], base[NUM_OUTPUT];  /////////////////////////////////// multi 2
  for(int i=0; i < NUM_OUTPUT; i++)
  {
    base[i]               = "test_results/" + i + scheduler_string + "_" + std::to_string(noUEs);
    log_file_name[i]      = base[i] + "_training.txt";
    model_name[i]         = base[i] + "_model.pt";
  }
  ...
  std::vector<experience> samples[4];  /////////////////////////////////// multi 3
  ...
  for(int ada_index=0; ada_index < ADA_ACTIONS; ada_index++)
  { //ada training
  printf("HH] training ada_index: %d\n", ada_index);
  bool explore = true;
  std::ofstream output_file(log_file_name[ada_index]);

/*
  for(int action_reset=0; action_reset < NUM_OUTPUT; action_reset++)
  {
      action.index_put_({0}, 0);
      action.index_put_({1}, 0);
      action.index_put_({2}, 0);
      action.index_put_({3}, 0);
      action.index_put_({4}, 0);
  }
*/
  while(1){ // training loop

  	torch::Tensor state = networkEnv->CurrentState();
  	networkEnv->TTI_increment();

  	// selecting an action
    torch::Tensor action = torch::zeros(4);

    if(use_dqn){ // select action explore/exploit in dqn
      action = agent->selectAction(state.to(device), policyNet, ada_index);
    } else { // use fixed scheduler
      action.index_put_({0}, 10);
      action.index_put_({1}, 0);
      action.index_put_({2}, 0);
      action.index_put_({3}, 0);
      action.index_put_({4}, 0);
    ...
    // execute action
    //HH
    //printf(" parameter A(%d) / B(%d) / C(%d) / D(%d)\n",
    ...
    torch::Tensor next_state  = networkEnv->CurrentState();
    torch::Tensor reward = networkEnv->CalculateReward(); 

    reward_copy = reward[0].item<float>();
    // store experiece in replay memory
    printf("here1\n");
    exp[ada_index]->push(state.to(torch::kCPU), action.to(torch::kCPU), next_state.to(torch::kCPU), reward.to(torch::kCPU)); 
printf("here2\n");
    start = std::chrono::steady_clock::now(); //training time logging
    if(use_dqn){ // if we are using dqn
    	// if enough samples
	    if(exp[ada_index]->canProvideSamples((size_t)MIN_REPLAY_MEM)){ 
        update_counter++;
        printf("here5\n");
        ...
        auto options = static_cast<torch::optim::SGDOptions&> (optimizer.defaults());
         ...
         printf("here3\n");
	    	...
	    	samples[ada_index] = exp[ada_index]->sampleMemory(BATCH_SIZE); 
printf("here4\n");
        experience batch = processSamples(samples[ada_index]);
        printf("process samples\n");
        // work out the qs
  ...
  if(use_dqn){ // only testing loops for DQN
    torch::save(policyNet, model_name[ada_index]);
    log_file_name[ada_index] = base[ada_index] + "_training.txt";
    output_file.open(log_file_name[ada_index]);
    ...
    while(1){ // testing loop
      ...
      torch::Tensor action = agent->exploit(state.to(device), policyNet, ada_index);
