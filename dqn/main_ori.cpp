...
int main(int argc, char** argv) {
  ...
  if(use_dqn){ // only testing loops for DQN
    ...
    while(1){ // testing loop
      ...
      torch::Tensor action = agent->exploit(state.to(device), policyNet);

    	// execute action
      //HH
      printf("SendScheduler2: %d\n", action[0].item<int>());
...
//HH
//#include "../src/device/ENodeB.h"
//#include "../src/flows/QoS/QoSParameters.h"
//#include "../src/componentManagers/NetworkManager.h"
//#include "../src/componentManagers/FlowsManager.h"

void DQN_Scheduler(){
  //int scheduler;
  //UpdateAllScheduler(FetchScheduler(scheduler));
}


//ENodeB::DLSchedulerType FetchScheduler(int scheduler){
...
/*
  printf("LTESIM: Received scheduler: %d\n", scheduler);
  ENodeB::DLSchedulerType downlink_scheduler_type;
...
/*
void UpdateAllScheduler(ENodeB::DLSchedulerType new_scheduler){
  // vector of enbs
