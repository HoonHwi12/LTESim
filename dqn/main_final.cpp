...
const int TRAIN_TTI         = 7500;//7500;
const int TEST_TTI          = 2500;//2500;
const int MIN_REPLAY_MEM    = 2000;//1000;
...
const int MEM_SIZE          = 500000;//50000; // replay memory size
...
//Adaptive DQN
const int ADA_ACTIONS       = 101;    // 0~10 * 0.1
...
*/
  ...
  torch::optim::SGD optimizer3(policyNet3->parameters(), torch::optim::SGDOptions(LR_START).momentum(MOMENTUM));
  ...
  while(1){ // training loop
    ...
    if(use_dqn){ // if we are using dqn
        ...
        torch::Tensor loss1 = (torch::mse_loss(current_q_values1.to(device), target_q_values1.to(device))).to(device);
        torch::Tensor loss2 = (torch::mse_loss(current_q_values2.to(device), target_q_values2.to(device))).to(device);
        torch::Tensor loss3 = (torch::mse_loss(current_q_values3.to(device), target_q_values3.to(device))).to(device);
  ...
  if(use_dqn){ // only testing loops for DQN
    ...
    while(1){ // testing loop
      ...
      torch::Tensor action = agent->exploit(state.to(device), policyNet0, policyNet1, policyNet2, policyNet3);
