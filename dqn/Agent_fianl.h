...
class Agent{
	...
	public:
		...
		torch::Tensor exploit(torch::Tensor state, R policy_net0, R policy_net1, R policy_net2, R policy_net3){
			...
			//std::cout << "after argmax: " << at::argmax(output0, 1) << std::endl;
		...
		torch::Tensor selectAction(torch::Tensor state, R policy_net0, R policy_net1, R policy_net2, R policy_net3){
			...
			if(exploration_rate > random_rate){ // explore
				...
				return exploit(state, policy_net0, policy_net1, policy_net2, policy_net3);
			}
		}
