...
ENodeB::DLSchedulerType Simulator::FetchScheduler(int *fd){
  ...
  *fd = open(SCHED_FIFO, O_RDONLY);
  ...
  d_dqn_output0 = 5*atoi(c_read_ptr);
  c_read_ptr = strtok(NULL,"|");  
  d_dqn_output1 = 5*atoi(c_read_ptr);
  c_read_ptr = strtok(NULL,"|");  
  d_dqn_output2 = 5*atoi(c_read_ptr);
  c_read_ptr = strtok(NULL,"|");  
  d_dqn_output3 = 5*atoi(c_read_ptr);
